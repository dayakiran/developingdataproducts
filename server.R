...
# Variables to use when a sample is added.
meanVar <- NULL
means_offset <- 0
...
shinyServer(function(input, output) {
  ...
  # Reactive expression to update the list of meanVar when the distribution changes
  doReset <- reactive({
    dist <- switch(input$dist,
                   norm = rnorm,
                   unif = runif,
  ...
  output$plot <- renderPlot({
    ...
    # Add to list of sample meanVar or repeat sampling reps times depending on checkbox
    if (input$checkbox) {
      if (length(meanVar)==0) {means_offset<<-input$resample}
      meanVar[input$resample-means_offset+1]<<-mean(x)
    }
    else {
      meanVar<<-1:reps
      for (i in 1:reps) {
        meanVar[i] <<-mean(switch(dist,
                                norm = rnorm(n),
                                unif = runif(n),
                                lnorm = rlnorm(n),
                                exp = rexp(n),
    ...
    xrm[x>xmax]<-NA
    xrm[x<xmin]<-NA
    meanVar[meanVar>xmax]<<-NA
    meanVar[meanVar<xmin]<<-NA
    ...
    # plot list of sample meanVar with the latest sample highlighted and N(mu,sigma^2/n)
    ...
    y0 = dnorm(x0,switch(dist,
               ...
               switch(dist,
                      ...
                      exp = 1/sqrt(n),
                      0))
    y0=y0/sum(y0)*length(meanVar)*mean(diff(breaks_mh))/mean(diff(x0))
